...
- Pin `UV_INDEX_URL=https://pypi.org/simple` explicitly in the versions build stage, and bake `UV_EXTRA_INDEX_URL=https://pypi.org/simple` into the offline image env and `setup-caches.sh`'s air-gap block. uv keys cached registry metadata by index URL, so air-gapped installs that replace `UV_INDEX_URL` with their own private mirror couldn't see anything cached under `pypi.org` — including the baked `uvx copier` warm-up — and `uvx copier` (run by `dr start`) failed with "packages must be downloaded from a registry". Pinning the build-time index makes what the cache is warmed against explicit rather than relying on the build host's ambient uv config; keeping `pypi.org` as an extra index at runtime keeps it queryable from cache alongside whatever mirror `UV_INDEX_URL` points at.
