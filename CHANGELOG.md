...
## 0.1.4
- Bump the default `datarobot` version from `3.18` to `3.19.0` (and the kernel venv's matching pin), the first release with the `dr-dev` shim that lets `dr` discover drdev as the `dev` plugin. A recipe that doesn't pin its own `datarobot` minimum-version in `versions.yaml` was silently getting an offline image without `dr dev`.
